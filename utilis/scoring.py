...
def bc_scoring(y, p):
    ...
    # Calculate F1 score
    precision = ppv(TP, FP, P)
    recall = tpr(TP, FN)
    f1 = 2 * (precision * recall) / (precision + recall)

    # Ensure f1 is a 2D tensor (add a new axis)
    f1 = f1.unsqueeze(0)

    # Concatenate F1 score to the scores tensor
    scores = pt.cat([scores, f1], dim=0)

    return scores  # ['acc','ppv','npv','tpr','tnr','mcc','auc','std', 'f1']
