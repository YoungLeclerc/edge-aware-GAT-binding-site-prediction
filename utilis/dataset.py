...
def select_by_sid(dataset, sids_sel):
    # extract sids（stucture ids） of dataset
...
def select_by_max_ba(dataset, max_ba):
    ...
    # create selection mask    ba（bio assembly）
    m = (aids <= max_ba)
...
def select_complete_assemblies(dataset, m):
    # get non-selected subunits 过滤不完整结构
...
def select_by_interface_types(dataset, l_types, r_types):
    # get types id        label的划分机制，l_typers是蛋白质残基，r_types为五类，对应五类标签
    t0 = np.where(np.isin(dataset.mids, l_types))[0]
    t1 = np.where(np.isin(dataset.mids, r_types))[0]
    print(f"t0 size: {t0.shape}, t1 size: {len(t1)}")
...
def load_sparse_mask(hgrp, k):    # get shape
...
def collate_batch_features(batch_data, max_num_nn=64):
    ...
    return X, ids_topk, q, M    # 将batch样本拼接为张量，原子坐标：X   ids_topk：最近邻索引  q：原子特征向量  M:残基掩码
...
class StructuresDataset(pt.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.with_preprocessing:
            # process structure
            structure = clean_structure(structure)  #去水分子，多余结构

            # update molecules chains
            structure = tag_hetatm_chains(structure) #将配体、离子等HETATM的chain tag 提取出来，用与分类识别
