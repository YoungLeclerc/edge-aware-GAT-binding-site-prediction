...
def encode_structure(structure, device=pt.device("cpu")):
    ...
    return X, M            #X: [N, 3]，每个原子的坐标   M: [N_res, N]，mask矩阵，表示哪些原子属于同一个残基
...
def structure_to_data(structure, device=pt.device("cpu")):
    ...
    # 新增：用于 GATConv
    from utilis.model_operations import unpack_state_features
    _, edge_index, edge_attr = unpack_state_features(X, ids_topk, q)

    return X, ids_topk, q, M, edge_index, edge_attr


def locate_contacts(xyz_i, xyz_j, r_thr, device=pt.device("cpu")):
    with pt.no_grad():
        # send data to device   xyz_i是链i所有原子的坐标，xyz_j是j链所有原子的坐标
...
def extract_graph_edges(X, ids_topk):
    # 计算方向与距离（用于构建 edge_attr）
    R_nn = X[ids_topk - 1] - X.unsqueeze(1)  # [N, k, 3]
    D_nn = pt.norm(R_nn, dim=2)              # [N, k]

    # 避免除以 0
    D_nn = D_nn + pt.max(D_nn)*(D_nn < 1e-2).float()
    R_unit = R_nn / D_nn.unsqueeze(2)        # [N, k, 3]

    # 构建 edge_index
    N, k = ids_topk.shape
    src = pt.arange(N, device=X.device).unsqueeze(1).repeat(1, k).view(-1)
    dst = ids_topk.view(-1) - 1  # -1 去除 sink
    mask = dst >= 0
    edge_index = pt.stack([dst[mask], src[mask]], dim=0)

    # 构建 edge_attr = [distance, dir_x, dir_y, dir_z]
    edge_attr = pt.cat([
        D_nn.view(-1, 1),
        R_unit.view(-1, 3)
    ], dim=1)[mask]
